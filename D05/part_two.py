...
for line in puzzle_input:
    ...
    if one[0] == two[0]:
        ...
        pos_or_neg_x = 1 if one[0] < two[0] else -1
        pos_or_neg_y = 1 if one[1] < two[1] else -1
        for y, x in zip(range(one[1], two[1] + pos_or_neg_y, pos_or_neg_y),
                        range(one[0], two[0] + pos_or_neg_x, pos_or_neg_x)):
            points[y][x] += 1
